...
- SliceFx は Minimal API に近い形を保ちたい project を対象にしています。MediatR / `IPipelineBehavior` を積極的に採用しているプロジェクトは主対象ではありません。
...
## なぜ `slicefx` CLI を .NET local tool として提供するのか

local tool は repository ごとに version pin できます。`dotnet tool restore` 後の `slicefx routes` がどの machine でも同じ version で動きます。global tool はバージョンがずれやすく、project executable は tool version を build output と混ぜてしまいます。
